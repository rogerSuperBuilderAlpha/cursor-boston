...
async function handleDiscordCallback(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.redirect(
      new URL(`/profile?discord=success&data=${discordData}&uid=${state}`, request.url)
    );
  } catch (error) {
    ...
    return NextResponse.redirect(new URL("/profile?discord=error&message=unknown", request.url));
  }
}
