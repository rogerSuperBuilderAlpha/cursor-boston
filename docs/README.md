# Documentation index

Read docs in this order depending on your goal:

| Order | Doc | Who it’s for |
|-------|-----|----------------|
| 1 | [GET_STARTED.md](GET_STARTED.md) | First-time contributors, no coding background |
| 2 | [DEVELOPMENT.md](DEVELOPMENT.md) | Developers — setup, scripts, hooks, architecture |
| 3 | [FIRST_CONTRIBUTION.md](FIRST_CONTRIBUTION.md) | Step-by-step first pull request |
| 4 | [API.md](API.md) | API route reference |
| 5 | [RELEASING.md](RELEASING.md) | Maintainers — versioning and GitHub Releases |
| 6 | [SUPPLY_CHAIN.md](SUPPLY_CHAIN.md) | Security automation, dependencies, and trust signals |
| 7 | [VERCEL.md](VERCEL.md) | Why PRs do not deploy to Vercel; production-only settings |

**Operations / events** (not required for typical code contributions):

- [HACK_A_SPRINT_2026_OPS.md](HACK_A_SPRINT_2026_OPS.md)
- [CONTRIBUTOR_MERGE_CREDIT_BACKFILL.md](CONTRIBUTOR_MERGE_CREDIT_BACKFILL.md)

**Repository policy docs** (under `.github/`):

- [Contributing](../.github/CONTRIBUTING.md) · [Support](../.github/SUPPORT.md) · [Governance](../.github/GOVERNANCE.md) · [Security](../.github/SECURITY.md) · [Changelog](../CHANGELOG.md)
